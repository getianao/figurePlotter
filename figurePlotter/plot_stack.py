        ...
        if only_average:
            ...
            for configId in range(len(configs)):
                ...
                if ylim is not None and height > ylim[1]:
                    height = ylim[1] + 0.1
                label = format(
                    np.round(averageFunc(ys + ys2), decimals=decimals),
                    "." + str(decimals) + "f",
                )
            ...
            plt.bar(
                ...
                width,
                alpha=opacity,
                color=colorPalette2[0],
                edgecolor=(edgecolor if edgecolor else colorPalette[0]),
                linewidth=1,
                label=stack_labels[1],
                hatch=hatch,
                bottom=stack_ys_bottom,
            )
    else:
        if only_average:
            for configId in range(len(configs)):
                ...
                if ylim is not None and height > ylim[1]:
                    height = ylim[1] + 0.1
                label = format(
                    np.round(averageFunc(ys), decimals=decimals),
                    "." + str(decimals) + "f",
                )
                averageHeightDict[configs[configId]] = label
                if labelAverage:
            ...
            for configId in range(len(configs)):
                ...
                if last_ys is None:
                    plt.bar(
                        apps_index + (len(configs) - 1) * width / 2,
                        ys,
                        width * len(configs),
                        alpha=opacity,
                ...
                if plotAverage:
                    ...
                    if last_ys_avg is None:
                        plt.bar(
                            np.array([len(apps) + width])
                            + (len(configs) - 1) * width / 2,
                            [averageFunc(ys)],
                            width * len(configs),
                            alpha=min(opacity * 1.20, 1.0),
                            color=colorPalette[configId],
                            edgecolor=(
                                edgecolor if edgecolor else colorPalette[configId]
                            ),
                            linewidth=1,
                            hatch=hatch,
                        )
